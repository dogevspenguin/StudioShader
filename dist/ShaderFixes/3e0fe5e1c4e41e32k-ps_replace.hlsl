...
static const float CUMULUS_TOP = 6000+9000;
...
{
    float DEPTH_CONTROL = 1.0;
    int3 pixelCoord = int3(v0.xy, 0);
    float hwDepth = GlobalDepth.Load(pixelCoord).r;

    // Linearize Depth
    float sceneDepth = DEPTH_CONTROL / (hwDepth + 0.000001);
    // Reconstruct Ray
    float4 r0;
    float3 r1;
    ...
    float2 intersection = IntersectCloudLayer(camPos, rayDir, 6000, 6000+9000);
...
// If the fadeFactor is higher than the random hash, we keep the pixel.
intersection.x *= step(hash(v0.xy), fadeFactor);

    float tNear = intersection.x;
    float tFar = intersection.y+500*hash(v0.xy);
    ...
    if (tFar > tNear) 
    {
        ...
        t += marchStep * hash(v0.xy);

        float3 accumulatedColor = float3(0,0,0);
        float transmittance = 1.0;
        
        float noiseScale = 0.0001;
        float3 windUV = CB5.DirDist.xyz; 
        ...
        if (isMoon) {
             sunColor = float3(0.0, 0.0, 0.0); // Remove Moon Light
        }
        ...
        if (!isMoon) {
            ...
            if (sunH < 0.3) {
                ...
                if (sunH < 0.15) {
                    currentTint = lerp(redTint, goldTint, smoothstep(0.0, 0.15, sunH));
                } else {
	    ...
	    tcul = t;
            ...
            if (density > 0.001)
            {
                ...
                float3 cloudAlbedo = totalLight * cloudColor.xyz;

                float stepDensity = density * marchStep * 0.001; 
    ...
    o0.xyz =finalColor / max(finalAlpha, 0.001);//float3(tcul/400000,tcul/400000,tcul/400000);//finalColor / max(finalAlpha, 0.001);
    o0.w = (1-tcul/400000)*finalAlpha;//1;//(1-tcul/400000)*finalAlpha;
    oDepth =0.00; 
}